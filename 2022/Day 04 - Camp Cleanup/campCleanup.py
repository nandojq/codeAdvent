...
for pair in input_data.split("\n"):
    ...
    first_range = [x for x in range(int(range_1.split("-")[0]), int(range_1.split("-")[1])+1)]
    second_range = [x for x in range(int(range_2.split("-")[0]), int(range_2.split("-")[1])+1)]
    # Check if one range contains other
    if set(second_range) >= set(first_range) or set(first_range) >= set(second_range):
        count += 1
...
for pair in input_data.split("\n"):
    ...
    second_range = [x for x in range(int(range_2.split("-")[0]), int(range_2.split("-")[1])+1)]
    # Check overlap
    overlap = [x for x in first_range if x in second_range]
    if len(overlap) > 0:
        count_overlap += 1
