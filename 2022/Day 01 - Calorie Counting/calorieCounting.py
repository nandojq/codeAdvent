...
#  Divide into elf stock
stock_list = input_data.split("\n")
elf_stock = {}
current_cal_count = 0
current_elf = 1
for item in stock_list:
    if item == "":
        elf_stock["Elf {}".format(current_elf)] = current_cal_count
        current_elf += 1
        current_cal_count = 0
    else:
        current_cal_count += int(item)
elf_stock["Elf {}".format(current_elf)] = current_cal_count

# Get most calories
max_cal = max(elf_stock.values())
print("Max Calories: {}".format(max_cal))
...
# Sort by calorie count and sum last three (highest values)
highest_tree_sum = sum(sorted(elf_stock.values())[-3:])
print("Max 3 Calories sum: {}".format(highest_tree_sum))
