...
# Load input
with open("input_answer.txt", "r") as f:
    content = [line for line in f.readlines()]
 
# Pre-process input file
separator_ix = content.index("\n")
stack_lines = content[0:separator_ix-1]
crate_lines = []
for ix, stack_line in enumerate(stack_lines):
    crate_lines.append([stack_line[ix:ix + 4].replace(" ", "").replace("\n", "") for ix in range(0, len(stack_line), 4)])
crate_stack = {stack_ix+1: [] for stack_ix in range(len(crate_lines[-1]))}
for line in crate_lines:
    for ix, crate in enumerate(line):
        if crate == "":
            continue
        else:
            crate = crate.replace("[", "").replace("]", "")
            crate_stack[ix+1].append(crate)
for stack in crate_stack:
    crate_stack[stack].reverse()


# Process orders
# orders = content[separator_ix+1::]
# for order in orders:
#     operations = int(order.split(" ")[1])
#     from_stack = int(order.split(" ")[3])
#     to_stack = int(order.split(" ")[5].replace("\n", ""))
#     for operation in range(operations):
#         crate = crate_stack[from_stack].pop()
#         crate_stack[to_stack].append(crate)
#
# top_crates = "".join([crate_stack[stack][-1] for stack in crate_stack])
# print("Top crates are: {}".format(top_crates))
...
for order in orders:
    ...
    from_stack = int(order.split(" ")[3])
    to_stack = int(order.split(" ")[5].replace("\n", ""))
    # Take crate group and pop them in order on destination stack
    crate_group = crate_stack[from_stack][-operations:]
    for crate in crate_group:
        crate_stack[to_stack].append(crate)
    # Delete crates from intial stack
    for operation in range(operations):
        crate_stack[from_stack].pop()
