...
# Function to resolve round
def simulate_round(opponent_plays, i_play):
    round_score = 0
    if i_play == "X":
        round_score += 1
        if opponent_plays == "A":
            round_score += 3
        elif opponent_plays == "B":
            round_score += 0
        elif opponent_plays == "C":
            round_score += 6
    elif i_play == "Y":
        round_score += 2
        if opponent_plays == "A":
...
# Apply logic for each round
game_data = input_data.split("\n")
total_score = 0
for round_data in game_data:
    # Get round plays
    opponent_play = round_data.split(" ")[0]
    i_play = round_data.split(" ")[1]
    # Get round score
    total_score += simulate_round(opponent_play, i_play)
...
# Choose your play
def choose_play(opponent_play, wanted_outcome):
    if opponent_play == "A":
        if wanted_outcome == "X":
            return "Z"
        elif wanted_outcome == "Y":
            return "X"
        elif wanted_outcome == "Z":
            return "Y"
...
for round_data in game_data:
    # Get round plays
    opponent_play = round_data.split(" ")[0]
    wanted_outcome = round_data.split(" ")[1]
    # Choose your play
    i_play = choose_play(opponent_play, wanted_outcome)
    # Get round score
    total_score += simulate_round(opponent_play, i_play)
