...
for rucksack in input_data.split("\n"):
    # Separate two compartments
    first_comp = rucksack[0:int(len(rucksack)/2)]
    second_comp = rucksack[int(len(rucksack)/2)::]
    # Get repeated item
    rep_item = [x for x in first_comp if x in second_comp][0]
    # Get value
    item_value = string.ascii_letters.index(rep_item)+1
    value_sum += item_value
    # print("Repeated: {}, Value: {}".format(rep_item, item_value))

# Return
print("Sum of items is: {}".format(value_sum))
...
# Process rucksack
value_sum = 0
rucksack_list = input_data.split("\n")
rucksack_groups = [rucksack_list[ix:ix+3] for ix in range(0, len(rucksack_list), 3)]
value_sum = 0
for rucksack_group in rucksack_groups:
    # Find common badge
    badge = [x for x in rucksack_group[0] if (x in rucksack_group[1] and x in rucksack_group[2])][0]
    # Get badge value
    badge_val = string.ascii_letters.index(badge) + 1
    # Sum value
    value_sum += badge_val

# Return
print("Sum of badge values is: {}".format(value_sum))